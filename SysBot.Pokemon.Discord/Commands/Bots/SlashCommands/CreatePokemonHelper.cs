...
public static class CreatePokemonHelper
{
        ...
        Action<T>? postProcessing = null) where T : PKM, new()
    {
        ...
        // Get user roles for AutoOT check
        ...
        if (SysCordSettings.Manager != null && userRoles != null)
        {
            bool hasAutoOTRole = SysCordSettings.Manager.GetHasRoleAccess(nameof(DiscordManager.RolesAutoOT), userRoles);
            if (!hasAutoOTRole)
            {
                // User doesn't have AutoOT permission
                ignoreAutoOT = true;
            }
        }
        else
        {
            // No role manager or no roles - don't use AutoOT
            ignoreAutoOT = true;
        }

        // Process showdown set
        var processed = await Helpers<T>.ProcessShowdownSetAsync(showdownText, ignoreAutoOT: ignoreAutoOT, userRoles).ConfigureAwait(false);
