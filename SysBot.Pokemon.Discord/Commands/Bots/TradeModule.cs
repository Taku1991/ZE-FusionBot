        ...
        _ = Task.Run(async () =>
        {
            try
            {
                ...
                for (int i = 0; i < selections.Count; i++)
                {
                    ...
                    var (pk, error, set, legalizationHint) = await BatchHelpers<T>.ProcessSingleTradeForBatch(tradeContent, hasBatchCommandsPermission, hasAutoOTPermission);
    ...
    public async Task BatchTradeAsync([Summary("List of Showdown Sets separated by '---'")][Remainder] string content)
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                ...
                {
                    var (pk, error, set, legalizationHint) = await BatchHelpers<T>.ProcessSingleTradeForBatch(trades[i], hasBatchCommandsPermission, hasAutoOTPermission);
                    if (pk != null)
                    {
                        batchPokemonList.Add(pk);
                    }
    ...
    private async Task ProcessTradeAsync(int code, string content, bool isHiddenTrade = false)
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                ...
                // Use the Pokemon generated by ProcessShowdownSetAsync
