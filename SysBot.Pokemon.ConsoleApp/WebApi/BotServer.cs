...
using SysBot.Base;
using SysBot.Pokemon.ConsoleApp.WebApi.Models;
using static SysBot.Pokemon.ConsoleApp.WebApi.RestartManager;

namespace SysBot.Pokemon.ConsoleApp.WebApi;

public partial class BotServer(IBotHost host, int port = 8080, int tcpPort = 8081) : IDisposable
{
    ...
    private readonly IBotHost _host = host ?? throw new ArgumentNullException(nameof(host));
    ...
    public void Start()
    {
        ...
        try
        {
            ...
            catch (HttpListenerException ex) when (ex.ErrorCode == 5)
            {
                ...
                LogUtil.LogInfo("1. Run this application as root or with appropriate privileges", "WebServer");
                LogUtil.LogInfo($"2. Or use a reverse proxy to expose port {_port}", "WebServer");
            }
    ...
    private async Task<string> UpdateAllInstances(HttpListenerRequest request)
    {
        try
        {
            ...
            var session = await UpdateManager.StartOrResumeUpdateAsync(_host, _tcpPort, forceUpdate);
    ...
    private async Task<string> UpdateSingleInstance(HttpListenerRequest request, int port)
    {
        try
        {
            ...
            if (port == _tcpPort)
            {
                ...
                try
                {
                    var session = await UpdateManager.StartOrResumeUpdateAsync(_host, _tcpPort, false);
            ...
            var success = await UpdateManager.UpdateSingleInstanceAsync(_host, port, _cts.Token);
    ...
    private InstanceIdleInfo GetLocalIdleInfo()
    {
        ...
        {
            if (!source.IsRunning || source.IsPaused)
            {
                idleCount++;
            }
            else
            {
                ...
                {
                    Name = source.Bot?.Connection.Label ?? "Unknown",
        ...
        return new InstanceIdleInfo
        {
            ...
            TotalBots = botSources.Count,
            IdleBots = idleCount,
            NonIdleBots = nonIdleBots,
            AllIdle = idleCount == botSources.Count
        };
    }
    ...
    private static async Task<string> CheckForUpdates()
    {
        try
        {
            var (updateAvailable, _, latestVersion, _) = await HeadlessUpdateChecker.CheckForUpdatesAsync(false);
            var changelog = await HeadlessUpdateChecker.FetchChangelogAsync();
    ...
    private BotInstance CreateLocalInstance()
    {
        ...
        var botStatuses = botSources.Select(s => new BotStatusInfo
        {
            Name = s.Bot?.Connection.Label ?? "Unknown",
    ...
    private string GetBots(int port)
    {
        try
        {
            if (port == _tcpPort)
            {
                ...
                {
                    Bots = [.. botSources.Select(s => new BotInfo
                    {
                        Id = $"{s.Bot?.Config.Connection.IP ?? "unknown"}:{s.Bot?.Config.Connection.Port ?? 0}",
                        Name = s.Bot?.Connection.Label ?? s.Bot?.Config.Connection.IP ?? "Unknown",
                        RoutineType = s.Bot?.Config.InitialRoutine.ToString() ?? "Unknown",
                        Status = s.IsRunning ? (s.IsPaused ? "Paused" : "Running") : "Stopped",
                        ConnectionType = s.Bot?.Connection.GetType().Name ?? "Unknown",
                        IP = s.Bot?.Config.Connection.IP ?? "unknown",
                        Port = s.Bot?.Config.Connection.Port ?? 0
                    })]
                };
    ...
    private async Task<List<CommandResponse>> ExecuteCommandOnAllInstancesAsync(string command)
    {
        var tasks = new List<Task<CommandResponse?>>
        {
            ...
            Task.Run(() =>
            {
                ...
                if (localResult != null)
                {
                    localResult.InstanceName = _host.InstanceName;
                }
                return localResult;
            })
        };
    ...
    {
        // Direct call via IBotHost - no UI thread marshaling needed in headless mode
    ...
    private bool IsMasterInstance()
    {
        ...
        var configuredPort = _host.Config?.Hub?.WebServer?.ControlPanelPort ?? 8080;
    ...
    private static byte[]? GetIconBytes()
    {
        try
        {
            ...
            var exePath = Environment.ProcessPath ?? Environment.CurrentDirectory;
            ...
            var assembly = Assembly.GetExecutingAssembly();
            var iconStream = assembly.GetManifestResourceStream("SysBot.Pokemon.ConsoleApp.icon.ico");
            ...
            // No WinForms Icon.ExtractAssociatedIcon available in headless mode - return null
            return null;
        }
        catch (Exception ex)
        {
    ...
    private async Task<string> ExecuteLocalRemoteButton(string button, int botIndex = 0)
    {
        try
        {
            ...
            await connection.SendAsync(cmd, System.Threading.CancellationToken.None);
            ...
            foreach (var cmd in commands)
            {
                ...
                else
                {
                    ...
                    if (switchButton != null)
                    {
                        ...
                        await connection.SendAsync(command, System.Threading.CancellationToken.None);
    ...
    private static SysBot.Base.SwitchButton? MapButtonToSwitch(string button)
    {
        return button.ToUpper() switch
        {
            ...
            "PLUS" or "+" => SysBot.Base.SwitchButton.PLUS,
            "MINUS" or "-" => SysBot.Base.SwitchButton.MINUS,
            "LSTICK" or "LTS" => SysBot.Base.SwitchButton.LSTICK,
            "RSTICK" or "RTS" => SysBot.Base.SwitchButton.RSTICK,
            "HOME" or "H" => SysBot.Base.SwitchButton.HOME,
            "CAPTURE" or "SS" => SysBot.Base.SwitchButton.CAPTURE,
            "UP" or "DUP" => SysBot.Base.SwitchButton.DUP,
            "DOWN" or "DDOWN" => SysBot.Base.SwitchButton.DDOWN,
            "LEFT" or "DLEFT" => SysBot.Base.SwitchButton.DLEFT,
            "RIGHT" or "DRIGHT" => SysBot.Base.SwitchButton.DRIGHT,
            _ => null
        };
    }
