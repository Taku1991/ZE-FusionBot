...
public static class UpdateManager
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        lock (_lock)
        {
            ...
            if (existingState != null && !existingState.IsComplete)
            {
                ...
                _ = Task.Run(async () =>
                {
                    try
                    {
                        await ResumeUpdateAsync(host, currentTcpPort, existingState, cancellationToken);
                    }
                    catch (Exception ex)
                    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                await ExecuteUpdateAsync(host, currentTcpPort, forceUpdate, state, cancellationToken);
            }
            catch (Exception ex)
            {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (instance.ProcessId == Environment.ProcessId && host != null)
            {
                await StopLocalBotsAsync(host, cts.Token);
            }
            else
            {
            ...
            // Perform update
            await UpdateInstanceAsync(host, instance, safeVersion, cts.Token);
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            try
            {
                ...
                (updateAvailable, _, latestVersion, downloadUrl) = await HeadlessUpdateChecker.CheckForUpdatesAsync(false).WaitAsync(versionCts.Token);
            }
            finally
            {
                versionCts?.Dispose();
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            foreach (var instance in pending)
            {
                ...
                SaveState();
                await UpdateInstanceAsync(host, instance, state.TargetVersion, cancellationToken);
            }
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var instance in instances)
        {
            try
            {
                ...
                {
                    // Local instance
                    await StopLocalBotsAsync(host, cancellationToken);
                }
                else
                {
                    // Remote instance
        ...
        while (DateTime.UtcNow < endTime && !cancellationToken.IsCancellationRequested)
        {
            ...
            if (_state?.IdleProgress != null)
            {
                foreach (var instance in instances)
                {
                    ...
                    if (idleStatus != null)
                    {
                        await UpdateInstanceIdleStatus(host, instance, idleStatus, cancellationToken);
            ...
            else
            {
                ...
                foreach (var instance in instances)
                {
                    if (!await IsInstanceIdleAsync(host, instance, cancellationToken))
                    {
                        allStopped = false;
                        break;
                    }
    ...
    /// </summary>
    private static async Task UpdateInstanceIdleStatus(IBotHost? host, Instance instance, InstanceIdleStatus idleStatus, CancellationToken cancellationToken)
    {
        try
        {
            if (instance.ProcessId == Environment.ProcessId)
            {
                // Check local bots via IBotHost
                if (host == null)
                {
                    LogUtil.LogInfo("IBotHost is null for local instance - assuming no bots", "UpdateManager");
                ...
                await Task.Run(() =>
                {
                    var botSources = host.GetBotSources();
                    idleStatus.TotalBots = botSources.Count;
                    idleStatus.NonIdleBots.Clear();

                    var idleCount = 0;
                    foreach (var botSource in botSources)
                    {
                        if (!botSource.IsRunning || botSource.IsPaused)
                        {
                            idleCount++;
                        }
                        else
                        {
                            var botName = botSource.Bot?.Connection?.Label ?? "Unknown";
                            idleStatus.NonIdleBots.Add($"{botName}: Running");
                        }
                    }
    ...
    /// <summary>
    /// Stop local bots via IBotHost
    /// </summary>
    private static async Task StopLocalBotsAsync(IBotHost? host, CancellationToken cancellationToken)
    {
        if (host == null) return;
    ...
    /// </summary>
    private static async Task<bool> IsInstanceIdleAsync(IBotHost? host, Instance instance, CancellationToken cancellationToken)
    {
        try
        {
            if (instance.ProcessId == Environment.ProcessId)
            {
                ...
                {
                    var botSources = host.GetBotSources();
                    return botSources.All(b => !b.IsRunning || b.IsPaused);
                }, cancellationToken);
            }
            else
            {
        ...
        for (int retry = 0; retry < Config.MaxRetryCount; retry++)
        {
            try
            {
                ...
                if (instance.IsMaster)
                {
                    ...
                    // Install update via shell script and exit (Linux-compatible)
                    await Task.Run(() =>
                    {
                    ...
                    // Application will exit and be restarted by shell script
                    return;
                }
                else
                {
    ...
    private static string GetCurrentVersion()
    {
        try
        {
            return SysBot.Pokemon.PokeBot.Version;
        }
        catch
        {
            try
    ...
    private static string? ValidateAndSanitizePath(string path)
    {
        try
        {
            ...
            var baseDir = Path.GetDirectoryName(Environment.ProcessPath) ?? Environment.CurrentDirectory;
            var basePath = Path.GetFullPath(baseDir);

            // Ensure the path is within the expected directory or temp directory (port files and downloads)
    ...
    {
        // Extract original filename from URL (e.g., "ZE_FusionBot" from GitHub release)
        var uri = new Uri(downloadUrl);
        var originalFileName = Path.GetFileName(uri.LocalPath);

        // Use original filename without extension for Linux binary
        string tempPath = Path.Combine(Path.GetTempPath(), $"{Path.GetFileNameWithoutExtension(originalFileName)}_{Guid.NewGuid()}");
    ...
    /// <summary>
    /// Install update and restart application via shell script (Linux-compatible)
    /// </summary>
